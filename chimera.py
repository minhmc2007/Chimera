...
class ChimeraInstaller:
    ...
    def _setup_kernel_initramfs(self):
        """
        The live ISO's own /boot usually doesn't contain a real, installable
        kernel -- the live session actually boots from
        /run/archiso/bootmnt/arch/boot/x86_64/, mounted straight off the ISO,
        not copied into the squashfs root. A plain rsync of / therefore leaves
        the target with either no kernel at all, or a stale one paired with an
        initramfs built with the "archiso" mkinitcpio hook (which searches for
        the live medium/overlay, not a normal disk root). Either way,
        grub-mkconfig has nothing bootable to find afterwards. This pulls the
        real kernel off the live medium, strips the archiso hook out of the
        mkinitcpio config/presets, and rebuilds the initramfs from scratch.
        """
        ...
        search_patterns = ["/usr/lib/modules/*/vmlinuz", "/boot/vmlinuz-linux",
    ...
    def _write_bios_device_map(self):
        """
        arch-chroot bind-mounts the host's entire /dev into the chroot,
        including install media (USB/CD). grub-install's auto-generated
        device map (grub-mkdevicemap) enumerates whatever it sees in /dev
        at install time and can assign the wrong hd0/hd1 to the target disk
        while install media is still attached. At real boot time, with the
        install media gone, the BIOS renumbers drives and the *embedded*
        prefix search inside core.img -- built using the wrong mapping --
        fails, dropping to an interactive "grub>" shell even though
        grub.cfg is present and correct on disk. Writing an explicit
        device.map before grub-install forces hd0 to unambiguously mean
        the real target disk.
        """
        os.makedirs(f"{MOUNT_POINT}/boot/grub", exist_ok=True)
        with open(f"{MOUNT_POINT}/boot/grub/device.map", "w") as f:
            f.write(f"(hd0) {self.disk}\n")
    ...
    def install_bootloader(self):
        ...
        # Force a correct, unambiguous BIOS drive map before grub-install runs.
        # UEFI installs don't use device.map at all, so this is a no-op there.
        if not self.uefi:
            self._write_bios_device_map()
        ...
        # grub-install writes boot.img/core.img directly to raw disk sectors,
        # bypassing the filesystem layer entirely -- those writes sit in the
        # block device's page cache, separate from the ext4 cache that a
        # plain umount flushes. Force it out now so a VM power-off/reboot
        # right after install can't lose the boot sector.
        self.run_cmd(["sync"], check=False)
    ...
    def cleanup(self):
        ...
        self.run_cmd(["sync"], check=False)
        if self.disk:
            self.run_cmd(["blockdev", "--flushbufs", self.disk], check=False)
